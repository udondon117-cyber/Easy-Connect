      ...
      >
        ...
        <View style={styles.card}>
          ...
          <View style={styles.navRow}>
            <View style={[styles.navIcon, { backgroundColor: "rgba(255, 107, 107, 0.15)" }]}>
              <MaterialCommunityIcons name="music-box-outline" size={20} color={Colors.recordingRed} />
            </View>
            <View style={styles.navTextGroup}>
              <Text style={styles.navLabel}>音楽の自動検知</Text>
              <Text style={styles.navDesc}>マイクから流れている曲を自動で見つける</Text>
            </View>
            <TouchableOpacity
              onPress={() => updateSettings({ autoMusicDetection: !settings.autoMusicDetection })}
            >
              <Ionicons
                name={settings.autoMusicDetection ? "toggle" : "toggle-outline"}
                size={34}
                color={settings.autoMusicDetection ? Colors.accent : Colors.textMuted}
              />
            </TouchableOpacity>
          </View>
          ...
          {/* 【NEW】翻訳エンジンの切り替え */}
          <View style={styles.navRow}>
            <View style={[styles.navIcon, { backgroundColor: "rgba(255, 165, 0, 0.15)" }]}>
              <MaterialCommunityIcons name="translate" size={20} color="#FFA500" />
            </View>
            <View style={styles.navTextGroup}>
              <Text style={styles.navLabel}>翻訳エンジン</Text>
              <Text style={styles.navDesc}>subtitleを {settings.targetLanguage === "en" ? "English" : "日本語"} に翻訳する</Text>
            </View>
            <TouchableOpacity
              onPress={() => updateSettings({ translationEnabled: !settings.translationEnabled })}
            >
              <Ionicons
                name={settings.translationEnabled ? "toggle" : "toggle-outline"}
                size={34}
                color={settings.translationEnabled ? Colors.accent : Colors.textMuted}
              />
            </TouchableOpacity>
          </View>
          ...
          {/* 【NEW】オーバーレイの透明度（スケルトン調整） */}
          <View style={styles.navRow}>
            <View style={[styles.navIcon, { backgroundColor: "rgba(100, 100, 255, 0.15)" }]}>
              <Ionicons name="color-filter-outline" size={20} color="#6464FF" />
            </View>
            <View style={styles.navTextGroup}>
              <Text style={styles.navLabel}>オーバーレイの濃さ</Text>
              <Text style={styles.navDesc}>背面の透け具合（小さいほど透明）</Text>
            </View>
            <View style={styles.opacitySelector}>
              {[0.0, 0.15, 0.4].map((op) => (
                <TouchableOpacity
                  key={op}
                  style={[styles.opBtn, settings.overlayOpacity === op && styles.opBtnActive]}
                  onPress={() => updateSettings({ overlayOpacity: op })}
                >
                  <Text style={[styles.opBtnText, settings.overlayOpacity === op && styles.opBtnTextActive]}>
                    {op === 0.0 ? "透明" : op === 0.15 ? "薄" : "中"}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
